  ...
  try {
    ...
    const trades: Trade[] = parse(csvData, {
      ...
      cast: (value, context) => {
        ...
        if (['entry_price', 'stop_loss_price', 'exit_price', 'pnl_amount', 'quantity'].includes(context.column as string)) {
    ...
    const summary = Object.entries(groupedTrades).map(([symbol, trades]) => ({
      ...
      avg_pnl: trades.reduce((sum, trade) => sum + trade.pnl_amount, 0) / trades.length,
